...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (isNaN(productId) || typeof allProducts === "undefined") {
    productDetailContainer.innerHTML = '<p class="error-text">ID sản phẩm không hợp lệ hoặc dữ liệu chưa sẵn sàng.</p>';
    return;
  }
  ...
  function displayProductDetails(prod) {
    ...
    const versionHtml = prod.version ? `<span class="meta-version">Phiên bản: ${prod.version}</span>` : "";
    const statusHtml = prod.status ? `<span class="meta-status">${prod.status}</span>` : "";

    // ----- NÚT HÀNH ĐỘNG MỚI -----
    let actionSectionHtml = `<div class="product-actions new-actions">`;

    // Nút Copy link luôn có
    actionSectionHtml += `
    ...
    modal.innerHTML = `
      <div class="modal-content">
        ...
        <div class="contact-info">
          ...
          <p><strong>Zalo:</strong> 0988.654.321</p>
        ...
        <div style="margin-top:20px; text-align:center;">
          <small style="color:#aaa;">Nhắn tin ngay để được báo giá tốt nhất!</small>
        </div>
      </div>
    `;
  ...
  if (backButton) {
    // Lấy URL trang danh sách mà người dùng vừa ở (referrer)
    const referrer = document.referrer;
    let savedParams = "";

    if (referrer && referrer.includes("sanpham.html")) {
      const url = new URL(referrer);
      savedParams = url.search; // toàn bộ query string: ?category=...&page=...
      // Lưu vào sessionStorage để trang danh sách ưu tiên đọc lại
      sessionStorage.setItem("productListReturnState", savedParams);
    } else {
      // Nếu không có referrer (ví dụ: mở trực tiếp), lấy từ sessionStorage (trường hợp F5)
      const stored = sessionStorage.getItem("productListReturnState");
      if (stored) savedParams = stored;
    }

    // Cập nhật href của nút "Quay lại"
  ...
  if (addToCartBtn && product) {
    addToCartBtn.addEventListener("click", () => {
      if (typeof CartManager === "undefined") {
        alert("Lỗi: Không tìm thấy logic Giỏ hàng.");
        return;
      }
      const success = CartManager.addToCart(product.id, 1);
      if (success) {
...
// Toast duplicate function (giữ lại để tránh lỗi nếu gọi ngoài DOMContentLoaded)
function showToast(message) {
