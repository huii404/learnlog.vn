document.addEventListener("DOMContentLoaded", function () {
    ...
    function initVerticalMenu() {
        ...
        categories.forEach(cat => {
            const subCats = [...new Set(notesData
    ...
    window.handleMenuClick = function(cat, sub) {
        ...
        if (btnText) {
            btnText.innerText = (cat === 'all') ? "Tất cả" : `${cat}${sub !== 'all' ? ' > ' + sub : ''}`;
        }
    ...
    function renderAll() {
        ...
        const filtered = notesData.filter((n) => {
            const matchCat = (currentFilter.category === "all" || n.category === currentFilter.category);
            const matchSub = (currentFilter.subCategory === "all" || n.subCategory === currentFilter.subCategory);
        ...
        noteGrid.innerHTML = filtered.map(n => `
            <div class="note-card collapsible animate-fade-in" id="note-${n.id}">
                <div class="note-header">
                    <h3>
                        <small style="display:block; font-size:0.6em; color:#bb86fc;">${n.category} > ${n.subCategory}</small>
                        ${n.title}
                    </h3>
                ...
                <div class="note-body">
                    ...
                    <div class="code-snippet">
                        ...
                        <button class="copy-btn" onclick="copyToClipboard('code-${n.id}', this)" title="Copy">
